@extends('layouts.app')

@section('content')
<div class="container-fluid px-0">
    <!-- Header halaman -->
    <div class="d-flex align-items-center mb-4 pb-2 border-bottom">
        <a href="/criteria" class="btn btn-outline-secondary me-3">
            <i class="bi bi-arrow-left"></i> Kembali
        </a>
        <div>
            <h1 class="h2 mb-0">
                <i class="bi bi-pencil-square text-warning me-2"></i> Edit Kriteria
            </h1>
            <p class="text-muted mt-1">Ubah informasi kriteria untuk perhitungan TOPSIS</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-body p-4 p-md-5">
            <form action="/criteria/{{ $criterion->id }}" method="POST" id="editForm">
                @csrf
                @method('PUT')

                <div class="row g-4">
                    <!-- Kode Kriteria -->
                    <div class="col-md-6">
                        <label for="kode" class="form-label fw-semibold">
                            <i class="bi bi-tag me-1"></i> Kode Kriteria
                        </label>
                        <input type="text"
                               class="form-control form-control-lg @error('kode') is-invalid @enderror"
                               id="kode"
                               name="kode"
                               value="{{ old('kode', $criterion->kode) }}"
                               placeholder="Contoh: C1, KECEPATAN"
                               required>
                        <div class="form-text">Kode unik untuk identifikasi kriteria.</div>
                        @error('kode')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <!-- Nama Kriteria -->
                    <div class="col-md-6">
                        <label for="nama_kriteria" class="form-label fw-semibold">
                            <i class="bi bi-card-text me-1"></i> Nama Kriteria
                        </label>
                        <input type="text"
                               class="form-control form-control-lg @error('nama_kriteria') is-invalid @enderror"
                               id="nama_kriteria"
                               name="nama_kriteria"
                               value="{{ old('nama_kriteria', $criterion->nama_kriteria) }}"
                               placeholder="Kecepatan, Akurasi, dll."
                               required>
                        @error('nama_kriteria')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <!-- Bobot -->
                    <div class="col-md-6">
                        <label for="bobot" class="form-label fw-semibold">
                            <i class="bi bi-percent me-1"></i> Bobot
                        </label>
                        <input type="number"
                               step="0.01"
                               class="form-control form-control-lg @error('bobot') is-invalid @enderror"
                               id="bobot"
                               name="bobot"
                               value="{{ old('bobot', $criterion->bobot) }}"
                               placeholder="0 - 100"
                               min="0"
                               max="100"
                               required>
                        <div class="form-text">Nilai bobot (0–100). Semakin tinggi, semakin penting.</div>
                        @error('bobot')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <!-- Tipe -->
                    <div class="col-md-6">
                        <label for="tipe" class="form-label fw-semibold">
                            <i class="bi bi-arrow-left-right me-1"></i> Tipe Kriteria
                        </label>
                        <select class="form-select form-select-lg @error('tipe') is-invalid @enderror" id="tipe" name="tipe" required>
                            <option value="benefit" {{ old('tipe', $criterion->tipe) == 'benefit' ? 'selected' : '' }}>
                                ✅ Benefit (semakin besar semakin baik)
                            </option>
                            <option value="cost" {{ old('tipe', $criterion->tipe) == 'cost' ? 'selected' : '' }}>
                                ⚠️ Cost (semakin kecil semakin baik)
                            </option>
                        </select>
                        <div class="form-text">Benefit = nilai tinggi lebih baik, Cost = nilai rendah lebih baik.</div>
                        @error('tipe')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                </div>

                <!-- Tombol Aksi -->
                <div class="d-flex justify-content-between mt-5 pt-3">
                    <a href="/criteria" class="btn btn-outline-secondary btn-lg px-4">
                        <i class="bi bi-x-circle me-1"></i> Batal
                    </a>
                    <button type="submit" class="btn btn-primary btn-lg px-5 shadow-sm">
                        <i class="bi bi-check-circle me-2"></i> Perbarui Kriteria
                    </button>
                </div>
            </form>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>
    // Validasi tambahan: bobot tidak boleh negatif dan maksimal 100
    const bobotInput = document.getElementById('bobot');
    bobotInput.addEventListener('change', function() {
        let val = parseFloat(this.value);
        if (isNaN(val)) this.value = '';
        else if (val < 0) this.value = 0;
        else if (val > 100) this.value = 100;
    });
</script>
@endpush